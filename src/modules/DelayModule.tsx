...
export default function DelayModule({ id }: DelayModuleProps) {
  ...
  useEffect(() => { if (delayRef.current) delayRef.current.delayTime.setTargetAtTime(time, engine.ctx.currentTime, 0.01); }, [time]);
  useEffect(() => { if (feedbackRef.current) feedbackRef.current.gain.value = Math.min(0.95, feedback); }, [feedback]);
  useEffect(() => {
  ...
  return (
    <ModulePanel title="DELAY" subtitle="Tape Echo" accentColor={accentColor} width={170} badge="FX">
      <div className="flex gap-2 justify-around mb-3">
        <Knob value={time} min={0.05} max={2} onChange={setTime} label="Time" unit="s" size="md" color={accentColor} logarithmic />
      ...
      <div className="rounded mb-3 overflow-hidden" style={{ height: 32, background: '#08081a', border: '1px solid #1a1a30' }}>
        <svg width={150} height={32}>
          {Array.from({ length: 7 }).map((_, i) => {
            ...
            const h = Math.max(1, amp * 24);
            return (
              <rect key={i} x={x - 4} y={16 - h / 2} width={8} height={h} rx={2}
                fill={accentColor} opacity={Math.max(0.1, amp)} />
            );
          })}
        </svg>
      </div>
